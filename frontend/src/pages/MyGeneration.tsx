...
const MyGeneration = () => {
  ...
  return (
    <>
      <SoftBackdrop />
      <div className="mt-22 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
        {/* Header */}
        <div className="mb-8">
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="rounded-lg bg-white/6 border border-white/10 animate-pulse h-[260px]"
              ></div>
            ))}
          </div>
        )}
            ...
            {thumbnails.map((thumbnail: IThumbnail) => {
              ...
              return (
                ...
                >
                  ...
                  {/* ✅ CONTENT (OUTSIDE aspect-ratio box) */}
                  ...
                  >
                    ...
                    <Link
                      target="_blank"
                      to={`/preview?thubnail_url=${thumbnail.image_url}&title=${thumbnail.title}`}
                    >
